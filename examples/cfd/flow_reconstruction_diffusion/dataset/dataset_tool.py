...
def open_image_folder(source_dir, *, max_images: Optional[int]):
    ...
    if len(labels) == 0:
        ...
        if len(toplevel_indices) > 1:
            labels = {
                arch_fname: toplevel_indices[toplevel_name]
...
def main(
    ...
    transform: Optional[str],  # ["center-crop", "center-crop-wide"]
    resolution: Optional[Tuple[int, int]],  # WxH
):
...
def parse_tuple(string):
    try:
        ...
        parsed = tuple(map(int, string.split("x")))
        if len(parsed) != 2:
            raise ValueError
    except ValueError:
        msg = f"{string} is an invalid resolution format. It should be WxH (e.g., 512x512)"
...
parser.add_argument(
    "--source",
    help="Input directory or archive name",
    metavar="PATH",
    type=str,
    required=True,
)
...
parser.add_argument(
    "--max-images",
    help="Maximum number of images to output",
    metavar="INT",
    type=int,
)
